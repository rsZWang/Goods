...
class Updater(
    ...
    fun loadVersionInfo(onSuccess: ((VersionInfo) -> Unit)) {
        OkHttpClient().newCall(
            Request.Builder()
                .url( "https://dl.dropbox.com/s/a9sgpj23ovd3txz/version_info.json?dl=0")
                .build()
        ).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Timber.e("Request failed.")
//                UpdaterFailedException("IOException: ${e.message}")
            }

            override fun onResponse(call: Call, response: Response) {
                if (response.code != 200) {
                    Timber.e("Request failed.")
//                    UpdaterFailedException("Loading version info HttpException: response code: ${response.code}, message: ${response.message}")
                } else {
...
//                UpdaterFailedException("IOException: ${e.message}")
            ...
            override fun onResponse(call: Call, response: Response) {
                if (response.code != 200) {
                    Timber.e("Request failed.")
//                    UpdaterFailedException("Downloading apk HttpException: response code: ${response.code}, message: ${response.message}")
                } else {
